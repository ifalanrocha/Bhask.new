...
using namespace std;// Declaração ou Resumo das Sub Rotinas
double ler_A ( ) ;
double ler_B ( ) ;
double ler_C ( ) ;
...
{   case 1:  a = ler_A ( );    b = ler_B ( );     c = ler_C ( );
           if ( a == 0 )  {  cout << "\nErro!'A' deve ser diferente de zero!";                                   system("pause");   exit(0);  }
           else   {  delta = cal_Delta ( a, b, c );                         exibir ( a, b, c, delta );            }  break;
...
double cal_Delta ( double a, double b, double c) {double d = pow(b,2) - 4 * a * c;return d; }
double cal_x1 ( double a, double b, double delta ) {double x = (-b + sqrt(delta))/(2*a);return x; }
double cal_x2 ( double a, double b, double delta ) {double x = (-b - sqrt(delta))/(2*a);return x; }
